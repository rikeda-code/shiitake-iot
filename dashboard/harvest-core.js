...
async function weeklyZoomRenderMekaki() {
  ...
  const rows = containers.map(c => {
    const no = c.no;
    const startD = c.startDate ? parseDateStr(c.startDate) : null;
    const isJishaC = isJishaLot(c.lot);
    const match = mekakiRows.find(r => {
      if (isJishaC) {
        if (r.siteName !== '自社菌床') return false;
        if (r.lot !== c.lot) return false;
...
function renderLotTab() {
  ...
  const html = lotKeys.map((lot, lotIdx) => {
    ...
    return `<div style="margin-bottom:40px">
      ...
      <div style="margin-top:16px;background:#13151f;border-radius:10px;padding:12px 16px">
        <div style="font-size:0.78rem;color:#64748b;margin-bottom:8px">ロット別収穫累計 (g/菌床)</div>
        <div style="height:240px"><canvas id="lot-chart-${lotIdx}"></canvas></div>
      </div>
    </div>`;
  }).join('');
  ...
  requestAnimationFrame(() => {
    lotKeys.forEach((lot, lotIdx) => {
      buildWeeklyLineChart(`lot-chart-${lotIdx}`, groups[lot], weeklyState.targetData || null);
    });
  });
}
...
function initTabs() {
  ...
  weeklyBtn.textContent = 'ロット別累計収穫量';
...
const WEIGHT_LOT_TAB = 99; // virtual index for lot comparison sub-tab
...
function renderWeightLotContent(result) {
  if (!result || result.error) return `<div style="color:#f87171;font-size:0.85rem;padding:16px">データ取得エラー</div>`;
  if (!result.containers || result.containers.length === 0) return `<div style="color:#64748b;padding:16px">データなし</div>`;
  // Group containers by lot
  const lotMap = {};
  ...
  const lots = Object.keys(lotMap).sort();
  const maxRound = Math.max(...result.containers.map(c => c.measurements.length), 1);
  return `
    <div style="font-size:0.8rem;color:#64748b;margin-bottom:12px">ロット別 散水前後重量比較（平均値, g）</div>
    <div style="height:${Math.max(280, 80 + lots.length * 44)}px;background:#13151f;border-radius:10px;padding:12px">
      <canvas id="weight-lot-chart"></canvas>
    </div>
    <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:12px;font-size:0.78rem;color:#64748b">
      ${lots.map(lot => {
        const cs = lotMap[lot];
        return `<span style="color:#f59e0b;font-weight:600">${lot}</span>: ${cs.length}コンテナ`;
      }).join(' / ')}
    </div>`;
}

function drawWeightLotChart(result) {
  if (!result || !result.containers || result.containers.length === 0) return;
  const canvas = document.getElementById('weight-lot-chart');
  if (!canvas) return;
  // Group by lot
  const lotMap = {};
  result.containers.forEach(c => {
  ...
  const lots = Object.keys(lotMap).sort();
  // For each lot, average before-weight and avg absorbed per measurement round
  const maxRound = Math.max(...result.containers.map(c => c.measurements.length), 1);
  const ROUND_COLORS = ['#60a5facc','#34d399cc','#f59e0bcc','#c084fccc','#fb923ccc','#f87171cc'];
  const datasets = [];
  for (let r = 0; r < maxRound; r++) {
    const beforeData = lots.map(lot => {
      const cs = lotMap[lot].filter(c => c.measurements[r] != null);
      if (!cs.length) return null;
      const avg = cs.reduce((s, c) => s + (c.measurements[r].before ?? 0), 0) / cs.length;
      return Math.round(avg);
    });
    const absorbData = lots.map(lot => {
      const cs = lotMap[lot].filter(c => c.measurements[r] != null && c.measurements[r].before != null && c.measurements[r].after != null);
      if (!cs.length) return null;
      const avg = cs.reduce((s, c) => s + Math.max(0, (c.measurements[r].after - c.measurements[r].before)), 0) / cs.length;
      return Math.round(avg);
    });
    if (beforeData.some(v => v != null)) {
      datasets.push({ label: `${r+1}本目 散水前`, data: beforeData, backgroundColor: ROUND_COLORS[r] || '#8892a4cc', stack: `s${r}` });
      datasets.push({ label: `${r+1}本目 吸水量`, data: absorbData, backgroundColor: (ROUND_COLORS[r] || '#8892a4cc').replace('cc','55'), stack: `s${r}` });
    }
  }
  new Chart(canvas.getContext('2d'), {
    type: 'bar',
    data: { labels: lots, datasets },
    options: {
      indexAxis: 'y',
      responsive: true, maintainAspectRatio: false,
      plugins: {
        ...
        tooltip: {
          backgroundColor: '#1e2235', borderColor: '#3a3f5c', borderWidth: 1,
          titleColor: '#e2e8f0', bodyColor: '#a0aec0',
          callbacks: { label: ctx => `${ctx.dataset.label}: ${(ctx.parsed.x||0).toLocaleString()}g` },
        },
      },
      scales: {
        x: { stacked: true, ticks: { color: '#8892a4', callback: v => `${v.toLocaleString()}` }, grid: { color: '#1e2235' } },
        y: { stacked: true, ticks: { color: '#e2e8f0', font: { weight: 'bold' } }, grid: { color: '#1e2235' } },
      },
    },
  });
}
...
function renderWeightSubTabs() {
  ...
  const allTabs = [
    ...WEIGHT_SITES.map((s, i) => ({ label: s.name, idx: i, color: s.color })),
    { label: 'ロット別比較', idx: WEIGHT_LOT_TAB, color: '#a78bfa' },
  ];
  const subTabHtml = allTabs.map(t => {
    const active = t.idx === weightActiveSite;
    return `<button onclick="weightSetSite(${t.idx})" style="
  ...
  let content;
  if (weightActiveSite === WEIGHT_LOT_TAB) {
    content = renderWeightLotContent(weightData[0]);
  } else {
    content = renderWeightSiteContent(weightData[weightActiveSite]);
  }
  ...
  section.innerHTML = `
    ...
    <div id="weight-content">${content}</div>`;

  if (weightActiveSite === WEIGHT_LOT_TAB) {
    requestAnimationFrame(() => drawWeightLotChart(weightData[0]));
  } else {
