/**
 * 収穫実績集計表(群馬) 新規タブ自動追加 Google Apps Script
 *
 * 「26/群馬担当号機収穫管理」スプレッドシートに紐づけて設置する。
 * 号機ごとのタブ(例:「17(7/22)L3」「3-17(7/19)L3」)を新規追加・リネームすると、
 * 「収穫実績集計表」シートの3行目(見本行)の下に1行追加し、
 * A~Q列に3行目と同じ関数を展開したうえで、F列に新規タブ名を記入する。
 * 逆に、追加されていたタブが削除されると、対応する集計行も自動的に削除する。
 *
 * いなべ版(dashboard/harvest-summary-automation.gs)と同じ仕組みだが、
...
// ロットタブと判定する命名パターン(例:「17(7/22)L3」「3-17(7/19)L3」)。
...
// シート複製直後の仮の名前(例:「群馬原本のコピー」「シート2」)はまだ最終的な
...
function removeSummaryRowByName() {
  ...
  const response = ui.prompt('削除するタブ名を入力してください(例: 17(7/22)L3)');
